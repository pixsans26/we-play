...
class TaskScratchScreen extends StatefulWidget {
  const TaskScratchScreen({super.key});

  @override
  State<TaskScratchScreen> createState() => _TaskScratchScreenState();
}

class _TaskScratchScreenState extends State<TaskScratchScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadScratchCounts() async {
    ...
    if (cp == null) return;
    final uidA = cp.partnerAUid;
    final uidB = cp.partnerBUid ?? 'partner_b_pending_${cp.id}';
    final histA = await ApiService.fetchHistory(uidA);
    final histB = await ApiService.fetchHistory(uidB);
    if (!mounted) return;
    setState(() {
      _scratchCountA = histA.where((h) => h['taskType'] == 'text' && h['completed'] == true).length;
      _scratchCountB = histB.where((h) => h['taskType'] == 'text' && h['completed'] == true).length;
    });
  }
  ...
  Future<void> _loadNextTask() async {
    ...
    final uid = game.currentTurn == 'A'
        ? auth.coupleProfile!.partnerAUid
        : (auth.coupleProfile!.partnerBUid ?? 'partner_b_pending_${auth.coupleProfile!.id}');

    final task = game.getNextTextTask(uid, _currentLevel);
    if (mounted) {
      setState(() {
        _currentTask = task;
        _noTasksRemaining = task == null;
        _isLoading = false;
      });
    }
  }
  ...
  void _onScratchComplete() {
    ...
    setState(() { _isScratched = true; _showConfetti = true; _isCompleted = false; });
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading && !_showPrevious) {
      return GameLoadingScaffold(bgColors: bgColors);
    }
    if (_noTasksRemaining) {
      return GameEmptyState(bgColors: bgColors, onBack: () => Navigator.of(context).pop());
    }
    if (_showPrevious && _previousTask != null) {
      return _PreviousTaskView(
        task: _previousTask!,
        bgColors: bgColors,
        onBack: () => setState(() => _showPrevious = false),
      );
    }
        ...
        children: [
          ...
          AnimatedBuilder(
            ...
            builder: (_, __) => Stack(children: [
              Positioned(top: 80, left: -30, child: Transform.translate(
                offset: Offset(0, -20 * _bgAnim.value),
                child: Transform.rotate(angle: -0.26, child: const Icon(Icons.favorite, size: 130, color: Color(0x14FFFFFF))),
              )),
              Positioned(top: 260, right: -20, child: Transform.translate(
                offset: Offset(0, 28 * _bgAnim.value),
                child: Transform.rotate(angle: 0.35, child: const Icon(Icons.favorite, size: 90, color: Color(0x0FFFFFFF))),
              )),
            ]),
          ),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text('Love Missions', textAlign: TextAlign.center, style: GoogleFonts.nunito(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900, shadows: [const Shadow(blurRadius: 4, offset: Offset(0, 1))])),
                      ),
                  ...
                  // ── Score Card ──
                  GameScoreCard(
                    partnerAName: cp?.partnerAName ?? 'A',
                    partnerBName: cp?.partnerBName ?? 'B',
                    avatarA: cp?.partnerAAvatar,
                    avatarB: cp?.partnerBAvatar,
                    countA: _scratchCountA,
                    countB: _scratchCountB,
                    turnName: turnName,
                  ...
                  Expanded(
                    child: Center(
                      child: _currentTask != null
                          ? _buildScratchArea(isDark, turnName, performingName)
  ...
  Widget _buildScratchArea(bool isDark, String turnName, String performingName) {
    final task = _currentTask!;
    final accentGrad = [const Color(0xFF7C3AED), const Color(0xFFDB2777)];

    return Stack(
      clipBehavior: Clip.none,
      children: [
        // Turn pill on top edge
        Positioned(
          ...
          child: Container(
            ...
            child: !_isScratched
                ? ScratchCard(
                    ...
                    overlayImage: isDark
                        ? const AssetImage('assets/images/overlay-dark.png')
                        : const AssetImage('assets/images/overlay-light.png'),
                    child: Stack(
                      fit: StackFit.expand,
                      children: [
                        Container(
                          decoration: BoxDecoration(
                            gradient: LinearGradient(colors: accentGrad, begin: Alignment.topLeft, end: Alignment.bottomRight),
                          ),
                          child: Column(
                            ...
                            children: [
                              ...
                              Padding(
                                ...
                                child: Text(task.title, textAlign: TextAlign.center, style: GoogleFonts.nunito(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900, shadows: [const Shadow(blurRadius: 4, offset: Offset(0, 2))])),
                              ),
                              const SizedBox(height: 8),
                              Padding(
                                padding: const EdgeInsets.symmetric(horizontal: 24),
                                child: Text(task.description, textAlign: TextAlign.center, style: GoogleFonts.nunito(color: Colors.white.withOpacity(0.9), fontSize: 15, fontWeight: FontWeight.w600)),
                              ),
                            ],
                          ),
                        ),
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Padding(
                                  padding: const EdgeInsets.symmetric(horizontal: 24),
                                  child: Text(task.title, textAlign: TextAlign.center, style: GoogleFonts.nunito(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900)),
                                ),
                                const SizedBox(height: 8),
                                Padding(
                                  padding: const EdgeInsets.symmetric(horizontal: 24),
                              ...
                              child: Column(
                                children: [
                                  if (_timerStarted) ...[
                                      ...
                                      builder: (_, __) => Opacity(
                                        ...
                                        child: Text(
                                          ...
                                          style: GoogleFonts.nunito(
                                            color: _timeLeft <= 10 ? Colors.red : (isDark ? Colors.white : const Color(0xFF4C1D95)),
                                            fontSize: 40,
                                    ...
                                    Text(
                                      _timerFinished ? '⏰ Time\'s up, $performingName!' : '$performingName must complete the task!',
                                      style: GoogleFonts.nunito(color: isDark ? Colors.white70 : const Color(0x99000000), fontSize: 13, fontWeight: FontWeight.w700),
                                    ),
                                  ] else
                                    Row(
                                      mainAxisAlignment: MainAxisAlignment.center,
  ...
  Widget _buildButtons(bool isDark) {
    ...
    return Column(
      children: [
        // Complete button
        Opacity(
          opacity: canComplete ? 1.0 : 0.35,
          child: GameBtn3D(
            label: 'Complete',
            colors: [const Color(0xFF9333EA), const Color(0xFFC026D3), const Color(0xFFDB2777)],
            shadowColor: const Color(0xFFE879F9),
            onTap: canComplete ? _handleDone : null,
          ),
        ),
        const SizedBox(height: 12),
        Row(
          children: [
            ...
            Expanded(
              child: Opacity(
                opacity: _previousTask != null && !_showPrevious ? 1.0 : 0.4,
                child: GameBtn3D(
                  label: 'Previous',
                  icon: Icons.arrow_back,
                  colors: [const Color(0xFF3B82F6), const Color(0xFF2563EB)],
                  shadowColor: const Color(0xFF93C5FD),
                  onTap: _previousTask != null && !_showPrevious ? () => setState(() => _showPrevious = true) : null,
                ),
              ),
            ),
            ...
            Expanded(
              child: Opacity(
                ...
                child: GameBtn3D(
                  label: 'Skip',
                  iconRight: Icons.skip_next,
                  colors: [const Color(0xFFFBBF24), const Color(0xFFF59E0B)],
                  shadowColor: const Color(0xFFFDE68A),
...
class _PreviousTaskView extends StatelessWidget {
  final Task task;
  final List<Color> bgColors;
  final VoidCallback onBack;

  const _PreviousTaskView({required this.task, required this.bgColors, required this.onBack});
    ...
    body: Container(
      ...
      child: SafeArea(
        child: Padding(
          ...
          child: Column(
            children: [
              Text('Previous Task (Read Only)', style: GoogleFonts.nunito(color: Colors.white70, fontSize: 14)),
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(task.emoji, style: const TextStyle(fontSize: 48)),
                      const SizedBox(height: 12),
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 24), child: Text(task.title, textAlign: TextAlign.center, style: GoogleFonts.nunito(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900))),
                      const SizedBox(height: 8),
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 24), child: Text(task.description, textAlign: TextAlign.center, style: GoogleFonts.nunito(color: Colors.white70, fontSize: 15))),
                    ],
                  ),
                ),
              ),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Center(child: Text('Back to Current Task', style: GoogleFonts.nunito(color: Colors.white, fontWeight: FontWeight.w800))),
                ),
              ),
            ],
          ),
