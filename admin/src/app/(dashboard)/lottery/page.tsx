...
interface LotteryItem {
  id: number; label: string; actionType: string;
...
export default function LotteryPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(editing ? "Lottery item updated successfully!" : "Lottery item created successfully!");
      setShowForm(false);
      setForm(emptyForm);
      setEditing(false);
      load();
    } catch (err) {
      toast.error("Failed to save lottery item.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string | number) => {
    try {
      ...
      toast.error("Failed to delete lottery item.");
    }
  };

  const handleEdit = (item: LotteryItem) => {
    setForm(item);
    setEditing(true);
    setShowForm(true);
  };
  ...
  const filtered = items.filter(t => 
    ...
    (!filterType || t.columnType === filterType) &&
  ...
  useEffect(() => { setPage(1); }, [search, filterType, filterLevel]);
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setForm(emptyForm); setEditing(false); setShowForm(true); }}
          className="flex items-center gap-2 bg-[#ea580c] hover:bg-orange-700 text-white font-semibold px-5 py-2.5 rounded-xl shadow-md shadow-orange-500/20 transition-all">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex flex-wrap gap-4 items-center">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search items..."
            className="w-full pl-11 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-orange-400" />
        </div>
        <div className="flex items-center gap-2 text-slate-500 font-medium text-sm">
          <Filter className="w-4 h-4" /> Filters:
        </div>
        <select value={filterType} onChange={e => setFilterType(e.target.value)}
          className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm text-slate-700 focus:outline-none focus:border-orange-400 min-w-[120px]">
        ...
        {(search || filterType || filterLevel) && (
          <button onClick={() => { setSearch(""); setFilterType(""); setFilterLevel(""); }} className="text-sm text-slate-400 hover:text-slate-700 flex items-center gap-1 font-medium transition-colors">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">Item Label</label>
                <input required value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} placeholder="e.g. Kiss, the neck, etc."
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-400 focus:ring-1 focus:ring-orange-400 transition-all" />
              </div>
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">Column Type</label>
                  <select required value={form.columnType} onChange={e => setForm(f => ({ ...f, columnType: e.target.value }))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-400 focus:ring-1 focus:ring-orange-400 transition-all">
                    <option value="action">Action</option>
                    <option value="spot">Spot</option>
                    <option value="extra">Extra</option>
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">Action Type</label>
                  <select required value={form.actionType} onChange={e => setForm(f => ({ ...f, actionType: e.target.value }))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-400 focus:ring-1 focus:ring-orange-400 transition-all">
              ...
              <div className="grid grid-cols-2 gap-5">
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" checked={form.active} onChange={e => setForm(f => ({ ...f, active: e.target.checked }))} className="w-5 h-5 text-orange-600 rounded" />
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={saving} className="flex-1 bg-[#ea580c] hover:bg-orange-700 text-white rounded-xl py-3 text-sm font-semibold shadow-sm flex items-center justify-center gap-2 transition-all disabled:opacity-70">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col">
          <DataTable
            ...
            onEdit={handleEdit}
            emptyMessage="No lottery items found matching filters."
            columns={[
              { key: "label", label: "Label", render: (t) => <span className="font-bold text-slate-800">{t.label}</span> },
              { key: "columnType", label: "Column", render: (t) => (
                <span className={`text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-md ${
                  t.columnType === "action" ? "bg-blue-100 text-blue-700" :
                  t.columnType === "spot" ? "bg-purple-100 text-purple-700" :
                  "bg-orange-100 text-orange-700"
                }`}>{t.columnType}</span>
              )},
              { key: "actionType", label: "Action Type", render: (t) => <span className="text-xs font-semibold text-slate-600 uppercase tracking-wider">{t.actionType}</span> },
