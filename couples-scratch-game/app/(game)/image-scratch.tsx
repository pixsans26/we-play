...
export default function ImageScratchScreen() {
  ...
  const loadScratchCounts = useCallback(async () => {
    ...
    try {
      ...
      const img = history.filter((h) => h.taskType === "image" && (h.completed === true || (h.completed as any) === 1 || String(h.completed) === "true" || String(h.completed) === "1"));
  ...
  const loadNextTask = useCallback(async (forcedTurn?: "A" | "B") => {
    ...
    try {
      ...
      if (activeTask) setPreviousTask(activeTask as ImageTask);
      setIsLoading(true);
      ...
      <LinearGradient colors={bgColors} locations={[0, 0.5, 1]} style={{ flex: 1 }}>
        <View style={{ flex: 1, paddingHorizontal: 20, paddingTop: 52, paddingBottom: 24 }}>
          ...
          <View style={{ flex: 1, marginVertical: 20, borderRadius: 24, borderWidth: isDark ? 0 : 4, borderColor: "#7c3aed" }}>
            <View style={{ flex: 1, borderRadius: 20, overflow: "hidden", backgroundColor: "#2e1065" }}>
              ...
              <View style={[S.cardOuter, { position: "relative", borderRadius: 30, borderWidth: isDark ? 0 : 6 }]}>
                ...
                >
                  ...
                  <Image
                    ...
                    style={[StyleSheet.absoluteFill, { backgroundColor: "#2e1065", margin: 10, borderRadius: 20 }]}
                    resizeMode="contain"
                  />
